...
The scheme dispatch is shared as `store.Classify(dsn) (Backend, target)` in the type-only parent
package. Deciding what a DSN means is separable from constructing a store, and only the decision
needs sharing — a helper that also *opened* would have to import both backends and close the cycle
issue 02 exists to prevent. `bot/store.go`'s `openStore` and the tool's `open` both go through it.

**Dry run against the real data, twice, in both directions:**

```
store-migrate: sqlite dryrun.db (schema 2) -> postgres /tts_dryrun (schema 2)
  commands                    3
  users                     114
  accounts                    0   (created on demand)
  ledger                 16,864   sequence -> 16,865
  settings                    3
  wordle_wins                 5
  connections_wins            1
  game_rounds                 0
verify: 114/114 balances match  ·  sum(delta) 20,089 == 20,089
        counts match  ·  leaderboard top-50 match
```

Postgres → a fresh SQLite file verified identically. A live insert on the destination after the copy
got id 16865, exactly `MAX(id)+1`.

**The verifier can fail**, checked by hand on the real copy and pinned as tests:

- one extra ledger row → `balance 247145913: source 4799, destination 4798`, plus the count, sum,
  max-id and leaderboard checks all firing; exit 1
- one deleted command → `commands row count: source 3, destination 2`; exit 1
- a second copy without `-force` → refused before touching anything

Refinement worth noting: the leaderboard mismatch line no longer appends a bare "(collation?)" to
every difference. It says so only when the two lists hold the *same rows in a different order*, which
is the shape the collation trap actually has — otherwise the hint fires on plain value mismatches and
sends the reader somewhere useless.

`TestSQLiteToSQLite` and `TestCopyAcrossBatchBoundary` (1,007 rows, across the 500-row batch) run
without Postgres, so the copier itself has coverage in a plain `go test ./...`.
