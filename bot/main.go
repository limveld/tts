...
func seedCommands(db CommandStore, logger *log.Logger) {
	...
	for _, c := range []store.Command{
		...
		{Name: "mazehelp", Response: "GET OUT!!!: !getout starts a round, first 5 to move get a seat. Move one square a turn with !up !down !left !right. Grab a key (there's one fewer than players) and reach the door. If a repeat move seems to vanish, say it another way: !go up or !gou. !mazewins for the leaderboard."},
