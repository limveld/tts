...
// mazeEventBatch caps how many events go into one INSERT, matching the SQLite
// twin so a batch that succeeds on one backend cannot fail on the other. Postgres
// also has a hard 65535-parameter ceiling per statement, and 200 rows x 11 columns
...
func (s *Store) MazeRoundLog(n int) ([]store.MazeRound, error) {
	rows, err := s.db.Query(
		`SELECT id, room_id, seed, started_at, ended_at, tick_ms, cycles, reason,
		        players, finishers, winner_id, winner_login, winner_display, input
		   FROM maze_rounds
		  -- id is TEXT, so this tie-break is a text comparison — and this cluster's
		  -- collation orders case- and punctuation-insensitively where SQLite compares
		  -- bytes. COLLATE "C" keeps the two backends returning the same order, which
		  -- the conformance suite checks. See store/postgres/store.go.
		  ORDER BY ended_at DESC, id COLLATE "C" DESC
		  LIMIT $1`, n)
	if err != nil {
		return nil, err
	}
...
func (s *Store) MazeRoundByID(id string) (store.MazeRound, bool, error) {
	rows, err := s.db.Query(
		`SELECT id, room_id, seed, started_at, ended_at, tick_ms, cycles, reason,
		        players, finishers, winner_id, winner_login, winner_display, input
		   FROM maze_rounds WHERE id = $1`, id)
	if err != nil {
...
func (s *Store) MazeRoundEvents(id string) ([]store.MazeEvent, error) {
	rows, err := s.db.Query(
		`SELECT round_id, seq, cycle, kind, seat, user_id, login, display, at, n, reason
		   FROM maze_events WHERE round_id = $1 ORDER BY seq`, id)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
