...
func testMazeTally(t *testing.T, s Store) {
	if wins, err := s.MazeAddWin("u1", "bob", "Bob"); err != nil || wins != 1 {
		t.Fatalf("first win: wins=%d err=%v", wins, err)
	}
	if wins, err := s.MazeAddWin("u1", "bobby", "Bobby"); err != nil || wins != 2 {
		t.Fatalf("after rename: wins=%d err=%v want 2", wins, err)
	}
	if _, err := s.MazeAddWin("u2", "amy", ""); err != nil {
		t.Fatal(err)
	}

	lb, err := s.MazeLeaderboard(10)
	if err != nil {
		t.Fatal(err)
	}
	if len(lb) != 2 || lb[0].Display != "Bobby" || lb[0].Wins != 2 {
	...
	if lb, _ := s.MazeLeaderboard(1); len(lb) != 1 {
		t.Errorf("MazeLeaderboard(1) returned %d rows", len(lb))
	}
}

// testTalliesAreIndependent is the case worth having now there are three of
// these. Each backend's maze.go was written by mirroring its wordle.go, and the
// way that goes wrong is a table name left behind in a copied query — which every
// single-game test above would still pass, because each one only ever looks at
// its own leaderboard.
//
// So this writes a different number of wins to each tally for the same user and
// insists all three disagree.
func testTalliesAreIndependent(t *testing.T, s Store) {
	for i := 0; i < 1; i++ {
	...
	cases := []struct {
		...
		{"wordle", func() (int, error) {
			lb, err := s.WordleLeaderboard(10)
			return firstWins(len(lb), func() int { return lb[0].Wins }), err
		}, 1},
	...
	for _, c := range cases {
		...
		if got != c.want {
			t.Errorf("%s tally = %d, want %d — the three tallies are reading each other's tables", c.name, got, c.want)
		}
	}
}

// firstWins reads the top row's win count, or 0 when the leaderboard is empty.
func firstWins(n int, top func() int) int {
	if n == 0 {
		return 0
	}
	return top()
